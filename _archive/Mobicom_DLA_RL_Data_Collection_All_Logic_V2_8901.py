...
subjectID = str('8901')
file = os.path.join(LOG_FOLDER, datetime.datetime.now().strftime(f"output_{subjectID}_%y%m%d_%H%M%S.log"))

logger = get_logger(subjectID)
...
#              ['Sahil', 'EP', '2', '9.1']] 
    ...
    for i in range(1, len(question_lib)+1):
        logger.info("----")
        logger.info(len(question_lib[str(i)]))
        for ind in range(1, len(question_lib[str(i)])+1):
            logger.info((question_lib[str(i)][str(ind)]["label"], question_lib[str(i)][str(ind)]["score"]))
...
#                         text_to_wav(speaker, therapist_resp)
    ...
    #                         logger.info((question_lib[str(item_number)]["1"]["label"].lower(), label.lower()))
...
#                                 therapist_resp =  "therapist_resp: " + therapist_resp
                            ...
                            logger.info("Have problem processing the response.")
                            logger.info((S, question_A, DLA_result[i], user_input[i]))
    ...
    #                         new_response.append({"item": S, "question": question_A, "DLA_result":DLA_result[i], "User_input":user_input[i]})
                            if len(DLA_result) == 1:
                                ...
                                logger.info(("10", question_text))
                        
                                log_question(question_text)
                                logger.info("11")

                                user_correction = get_resp_log()
                                logger.info(("12", user_correction))
                                ...
                                user_correction = get_resp_log()
                                logger.info(("15", user_correction))
                            ...
                            if "no" in user_correction:
                                ...
                                logger.info("in evaluate: check if need to rephrase the answer.")
                                if rephrase_count == 0:
                                    logger.info("Please rephrase.")
...
#         logger.info(("question_q_table", question_q_table))
#         logger.info(("new_question_q_table", new_question_q_table))
        is_terminated = False
...
#         if "Dimension "+str(issue_dimension_number) == dimension_name_number:
    ...
    logger.info((item_number, question_number, dimension_name))
...
#     logger.info('initialize the q tables')
...
#             user_correction = get_resp_log()
#             logger.info(type(user_correction))
#             user_correction = user_correction.replace(".", " ")
#             user_correction = user_correction.replace(",", " ")
#             user_correction = user_correction.replace("?", " ")
#             user_correction = user_correction.split(" ")
...
with open(filename, 'w') as f:
    logger.info(("clear the file for:", filename))
    json.dump(question_lib, f)
